...
HllBases = Union[
    ...
    temporal.MonthBase,
    temporal.YearBase,
    temporal.MonthLatLngBase,
    temporal.MonthHashtagBase,
    temporal.MonthHashtagLatLngBase,
    topical.TermBase,
    topical.EmojiBase,
    topical.TermLatLngBase,
    topical.HashtagLatLngBase,
...
class ProtoHLLMapping:
    ...
    def extract_hll_base_metrics(self, record, record_type) -> Optional[List[HllBases]]:
        ...
        if record_hll_metrics is None:
            return
        hll_bases = self.extract_hll_bases(record, record_type, self.include_lbsn_bases)
        # turn generator into list
        hll_base_metrics = list(
            ProtoHLLMapping.update_bases_metrics(
    ...
    def update_bases_metrics(
        bases: List[HllBases], metrics: hll.HllMetrics
    ) -> Iterator[HllBases]:
    ...
    def extract_hll_bases(
        self, record, record_type, include_lbsn_bases: List[str]
    ) -> List[HllBases]:
        ...
        if record_type == lbsn.Post.DESCRIPTOR.name:
            ...
            if include_lbsn_bases and spatial_base in include_lbsn_bases:
                base_records = self.make_base(
                    facet="spatial", base=spatial_base, record=record
                )
                if base_records:
            ...
            temporal_bases = self.filter_bases(temporal_bases, include_lbsn_bases)
            base_records = self.make_bases(
                facet="temporal", bases=temporal_bases, record=record
            )
            if base_records:
                base_list.extend(base_records)
            # Topical Facet
            ...
            topical_bases = self.filter_bases(topical_bases, include_lbsn_bases)
            base_records = self.make_bases(
                facet="topical", bases=topical_bases, record=record
            )
            if base_records:
                base_list.extend(base_records)
            social_bases = ["community"]
            social_bases = self.filter_bases(social_bases, include_lbsn_bases)
            base_records = self.make_bases(
                facet="social", bases=social_bases, record=record
            )
            if base_records:
                base_list.extend(base_records)
        ...
        if record_type == lbsn.Place.DESCRIPTOR.name:
            if include_lbsn_bases and "place" in include_lbsn_bases:
                base_records = self.make_base(
                    facet="spatial", base="place", record=record
                )
                if base_records:
                    base_list.extend(base_records)
        if record_type == lbsn.City.DESCRIPTOR.name:
            if include_lbsn_bases and "city" in include_lbsn_bases:
                base_records = self.make_base(
                    facet="spatial", base="city", record=record
                )
                if base_records:
                    base_list.extend(base_records)
        if record_type == lbsn.Country.DESCRIPTOR.name:
            if include_lbsn_bases and "country" in include_lbsn_bases:
                base_records = self.make_base(
                    facet="spatial", base="country", record=record
                )
                if base_records:
                    base_list.extend(base_records)
        return base_list
    ...
    @classmethod
    def filter_bases(cls, base_list: List[str], include_lbsn_bases: List[str]):
    ...
    def make_bases(cls, facet: str, bases: List[str], record):
        ...
        base_list = []
        for base in bases:
            base_records = cls.make_base(facet=facet, base=base, record=record)
            if base_records:
                base_list.extend(base_records)
        return base_list
    ...
    def make_base(cls, facet: str, base: str, record):
        ...
        base_records = hll.base_factory(facet=facet, base=base, record=record)
        return base_records
